...
private:
	...
	/**
	 * Interpolates between a and b with a linear function using alpha as
	 * parameter.
	 *
	 * @param a The first value for interpolation, if alpha = 0, this value is
	 * returned.
	 * @param b The second value for interpolation, if alpha = 1, this value is
	 * returned.
...
public:
	 ...
	 * The quadcopters in the from and to vectors have the position in the
	 * vector as id.<br />
	 ...
	 * @param errorSize The maximum size of a random error. A random error
	 * ranges from -errorSize to errorSize and is added to the position.
	 *                  For every component of the position vector a new random
	 * number is generated, but the random error is applied to
	 *                  either all components of the position vector or none.
	 * @param defaultErrorSize The maximum size of the error that is always
	 * applied.
	 *                         The default error ranges from -defaultErrorSize
	 * to defaultErrorSize.
	 * @param steps The amount of steps to use for moving from the start to the
	 * end position.
	 * @param stepsPerSecond The amount of steps to calculate per second.
	 */
	MovementGenerator(IPositionReceiver *receiver, std::vector<Vector> from, std::vector<Vector> to, double errorRate,
	                  double errorSize, double defaultErrorSize, int steps, double stepsPerSecond);
