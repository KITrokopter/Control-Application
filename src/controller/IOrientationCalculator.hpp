...
public:
	 ...
	 * Classes implementing this should keep track of the positions and movement
	 ...
	 * positions.at(i) is the position of the
	 * quadcopter with id ids.at(i) that has the target movement vector
	 * targetMovement.at(i). In the return value, the
	 * orientation saved at position i is the orientation of the quadcopter with
	 * id ids.at(i).
	 */
	virtual std::vector<Vector> getOrientation(std::vector<int> ids, std::vector<Vector> positions,
	                                           std::vector<Vector> targetMovement) = 0;
};
