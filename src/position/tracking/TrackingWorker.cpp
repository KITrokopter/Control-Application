 ...
 */
TrackingWorker::TrackingWorker(IPositionReceiver *receiver) : errorGraph(100,
	                                                                     "Difference"), latencyGraph(500, "Latency")
{
	assert(receiver != 0);
 ...
 * If there is enough, a new position is calculated and sent to the position
 * receiver.
 */
void TrackingWorker::run()
{
	...
	while (!stop) {
		...
		if (data.size() > 1) {
			...
			// ROS_DEBUG("Got info from camera %d: [%.2f, %.2f, %.2f]",
			// data[0].camNo, data[0].cameraVector.getV1(),
			// data[0].cameraVector.getV2(), data[0].cameraVector.getV3());
			...
			if (position.isValid()) {
				// Invert x axis for controller. Uncomment if necessary, but
				// only if you really thought about it.
 ...
 * If it returns false, it is does not always mean that there is really nothing
 * to return.
 * Classes implementing this should ensure that if there is good data available,
 * true is returned.
 *
 * @return True if data is available, false if it can not be ensured that data
 * is available.
 */
...
void TrackingWorker::sendPosition(Vector position, int quadcopterId)
{
	if (quadcopterPositionPublishers.count(quadcopterId) == 0) {
		...
		ros::NodeHandle n;
		quadcopterPositionPublishers[quadcopterId] =
		    n.advertise<control_application::quadcopter_position>(name.str(), 4);
	}
