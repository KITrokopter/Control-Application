 ...
 * Used method of:
 * M. Warren, D. McKinnon, B. Upcroft, "Online Calibration of Stereo Rigs for
 * Long-Term Autonomy", in International Conference on Robotics and Automation,
...
// Long-Term Autonomy", in International Conference on Robotics and Automation,
// Karlsruhe, Germany, 2013.
void AmccCalibration::multiCameraCalibration(int numberCameras, double squareLengthX, double squareLengthY,
                                             int numberSquareCornersX, int numberSquareCornersY)
{
	...
	// Where the images are (forward slashes only, and must include a trailing
	// slash)
	engEvalString(ep, "input_dir = '/tmp/calibrationImages/';");

	// Where the data will be saved (forward slashes only, and must include a
	// trailing slash). This folder should already exist
	...
	// The index of the cameras to calibrate. In this example we are calibrating
	// four cameras with sequential naming.
	...
	// indicate whether or not to use the fisheye calibration routine (not
	// strictly required).
	engEvalString(ep, "fisheye = false;");

	// indicate whether or not to use the third radial distortion term when
	// doing a projective calibration (not strictly required)
	...
	// the base naming convention for the calibration images (not strictly
	// required), will default to the 'camX_image' convention if not used.
	engEvalString(ep, cam_names.c_str());

	// indicate whether or not to use the batch mode of the stereo calibrator
