...
TrackingArea::TrackingArea(vector<Vector> cameraPosition, vector<Vector> cameraDirection, int numberCameras,
                           double maxRange)
{
...
bool TrackingArea::contains(Vector x)
{
	...
	if ((x.getV3() > up.getV3()) || (x.getV3() < low.getV3()) || (x.getV2() < a1.getV2()) || (x.getV2() > a2.getV2()) ||
	    (x.getV1() < a1.getV1()) || (x.getV1() > a3.getV1())) {
		return false;
	} else {
...
bool TrackingArea::inTrackingArea(Vector cameraPosition, Vector cameraDirection, Vector x)
{
	...
	// finding direction vectors of the plane of the floor of the camera range
	// pyramid.
	Vector u =
	    Vector(cameraDirection.getV1(), cameraDirection.getV2(),
	           -(cameraDirection.getV1() * cameraDirection.getV1() + cameraDirection.getV2() *
	             cameraDirection.getV2()) / cameraDirection.getV3());
...
double TrackingArea::binarySearch(double leftBorder, double rightBorder, double posChange, double height,
                                  double heightPos, double heightNeg, int value)
{
	...
	case 0: {
		...
		while (fabs(rightBorder - leftBorder) > 1) {
			...
			// checks whether all corners of tracking area are still tracked of
			// all cameras
...
// value = 0, posChange, value = 1, height, value = 2, heightPos, value = 3,
// heightNeg
...
void TrackingArea::setTrackingArea()
{
	...
	if (!(inCameraRange(center))) {
		...
		/**
		 * searching optimal height (down) of square where the square size is
		 * biggest
		 */
		...
		// if lower = true, then the optimal middlepoint of trackingarea is
		// lower than center
		bool lower = true;
		...
		if (lower) {
			...
			// maximal width of tracking area is between heightLower and
			// heightLower/2
			...
			while (-rightBorderHeight + leftBorderHeight > 1) {
				...
				middleHeight = leftBorderHeight + (rightBorderHeight - leftBorderHeight) / 2;
				// ROS_DEBUG("lower %.2f, maximal quadrat size is %.2f",
				// middleHeight, 2 * (oldSize));
			}
			ROS_DEBUG("Found optimal middlepoint, between %.2f and %.2f with size %.2f", rightBorderHeight,
			          leftBorderHeight, 2 * oldSize);
		}
		/**
		 * searching optimal height (up) of square where the square size ist
		 * biggest
		 */
		else {
			...
			while (rightBorderHeight - leftBorderHeight > 1) {
				...
				middleHeight = leftBorderHeight + (rightBorderHeight - leftBorderHeight) / 2;
				// ROS_DEBUG("upper %.2f, maximal quadrat size is %.2f",
				// middleHeight, 2 * (oldSize));
			}
			ROS_DEBUG("Found optimal middlepoint, between %.2f and %.2f with size %.2f", leftBorderHeight,
			          rightBorderHeight, 2 * oldSize);
		}
...
{
	ROS_DEBUG(
	    "Tracking area is from [%.2f, %.2f, %.2f] to [%.2f, %.2f, %.2f], square is of size %.2f, upper point is [%.2f, %.2f, %.2f], lower point is [%.2f, %.2f, %.2f].",
	    a1.getV1(), a1.getV2(), a1.getV3(), a3.getV1(), a3.getV2(), a3.getV3(), a1.add(a2.mult(
	                                                                                       -1)).getLength(),
	    up.getV1(), up.getV2(), up.getV3(), low.getV1(), low.getV2(), low.getV3());
	Vector c = center;
